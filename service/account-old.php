...
class account {
	...
	private function validateToken($token) {
		...
		if( $user = $this->getByIdAndToken($id, $token) ) {
			...
			if( $user->token_expires == 1 || $user->token_expires > time() ) {
				...
				if( ($user->token_expires - (5*60)) < time() ) {
	...
	private function createToken($user) {
		...
		if( $this->updateToken($user->id, $token, $expires) ) {
			$user->token = $token;
	...
	private function updateToken($id, $token=null, $expires=null) {
	...
	public function signOut() {
		return $this->setUserOffline($this);
	}
